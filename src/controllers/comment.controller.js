...
const addComment = asyncHandler(async (req, res) => {
    ...
    if(!comment){
        throw new ApiErrors(500,"something went wrong when adding your comment in database")
    }
...
const updateComment = asyncHandler(async (req, res) => {
    ...
    if(comment.owner !== req.user._id){
        throw new ApiErrors(405,"you are not owner of this comment, unable to edit")
    }

    const isCommentUpdated = await Comment.findByIdAndUpdate(commentId,{
        $set:{content}
    },{new:true})
