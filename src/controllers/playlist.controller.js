...
const createPlaylist = asyncHandler(async (req, res) => {
    ...
    if(!playlist){
        throw new ApiErrors(500,"something went wrong when creating your playlist")
    }

    return res
    .status(200)
    .json(new ApiResponse(200,playlist,"your playlist is created successfully"))
})
...
const addVideoToPlaylist = asyncHandler(async (req, res) => {
    ...
    if(playlistexist?.owner !== req.user._id){
        throw new ApiErrors(408,"You does not have permission to add video in this playlist")
    }
    ...
    const isAdded = await Playlist.findByIdAndUpdate(playlistId,
        {
            $addToSet:{
                videos:videoId
            }
    ...
    if(!isAdded){
        throw new ApiErrors(500,"something went wrong when adding your video to playlist")
    }

    return res
    .status(200)
    .json(new ApiResponse(200,isAdded,"video added to playlist successfully"))
})
...
const removeVideoFromPlaylist = asyncHandler(async (req, res) => {
    ...
    if(playlistexist?.owner !== req.user._id){
        throw new ApiErrors(408,"You does not have permission to remove video from this playlist")
    }

    if(!playlistexist?.videos.includes(videoId)){
        throw new ApiErrors(404,"The video you want to remove is not present in playlist")
    }
    ...
    if(!removed){
        throw new ApiErrors(500,"something went wrong when removing video from playlist")
    }
...
const updatePlaylist = asyncHandler(async (req, res) => {
    ...
    if(playlist?.owner.toString().equels(req.user._id.toString())){
        throw new ApiErrors(408,"You do not have permission to update this playlist ")
    }
