...
const refreshAccessToken = asyncHandler(
  async(req, (res) => {
    const incomingRefreshToken =
      req.cookies.refreshToken || req.body.refreshToken; //this 2 option for  mobile
    ...
    try {
      ...
      ); //this is done for getting user from id.
...
const getUserChannelProfile = asyncHandler(async (req, res) => {
  ...
  const channel = await User.aggregate([
    //aggregation pipeline return mostly array with object.
    {
      $match: {
        username: username?.toLowerCase(),
      }, //here give one document where username is that.
    },
    {
      $lookup: {
        from: "subscriptions",
    ...
    {
      //project give the projection or project selected things whom demanded it .
      $project: {
        fullName: 1,
        username: 1,
        subscribersCount: 1,
